...
import { getObraBySlug, getCidadesDisponiveis } from "@/lib/obras"
import { StatusBadge } from "@/components/obras/status-badge"
import { Badge } from "@/components/ui/badge"
import { Card, CardContent, CardHeader } from "@/components/ui/card"
import { Separator } from "@/components/ui/separator"
import { MarketplaceShell } from "@/components/layout/marketplace-shell"
import { CotacaoToggleButton } from "@/components/obras/cotacao-toggle-button"
...
export default async function ObraDetailPage({
  ...
  const { slug } = await params
  const [obra, cidades] = await Promise.all([
    getObraBySlug(slug),
            ...
            <div className="lg:col-span-1">
              <Card className="sticky top-24 border-border shadow-none rounded-none">
                ...
                <CardContent className="space-y-4">
                  {/* Cotação Toggle Action */}
                  <CotacaoToggleButton obra={obra} className="w-full py-2.5" />
